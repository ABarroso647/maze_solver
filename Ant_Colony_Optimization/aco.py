...
class Ant:
    def __init__(self, maze, pheremones, max_moves):
        self.position = [0, 0]
        ...
        self.ANT_MAX_MOVES_HYPER_PARAM = max_moves
        # The end of the maze (bottom right corner)
        self.GOAL_POSITION = [len(maze) - 1, len(maze) - 1]

    def move(self):
        while self.moves < self.ANT_MAX_MOVES_HYPER_PARAM:
            probabilities = []
            all_pheremones = 0
...
def run_ant_colony(maze, shortest_path_len, max_iterations, ant_count, max_moves):
    # Number of times ants are released into maze (generations)
    ANT_COLONY_MAX_ITERATIONS = max_iterations
    ...
    while iterations < ANT_COLONY_MAX_ITERATIONS:
        ...
        ants = [Ant(final_maze.maze, final_maze.pheremone_matrix, max_moves) for _ in range(ANT_COUNT)]
        for ant in ants:
            # Move each ant
            ant.move()
            ...
            if ant.completed_maze:
                ...
                if len(ant.current_path) < shortest_path_length:
                    ...
                    if (ENABLE_TIMING and not shortest_path_completion and
                            len(ant.current_path) == known_shortest_path_len):
